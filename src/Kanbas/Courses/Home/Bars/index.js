            ...
            {links.map((link, index) => (
                <div className="bars-item" >
                    <Link key={index} to={`/Kanbas/${link}`} >
                        <FontAwesomeIcon icon={icons[index]} className={`bars-item-icon align-self-center ${icons[index] === faCircleUser ? "grey-user" : ""} ${[faCircleUser, faBook, faClock, faCircleQuestion].includes(icons[index]) ? "icon-margin-left" : ""}`} />
                        <div className="bars-item-text align-self-center">{link}</div>
                    </Link>
